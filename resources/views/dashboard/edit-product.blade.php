...
@section('content')
    ...
    <div class="w-75" style="color: white; margin-top: 20px;background: #1f364d!important;margin-left: 75px">
        <div style="padding: 50px">
            <div class="d-flex">
                ...
                <div class="ml-4">
                    <h3 style="margin-top: 30px">Editing Product</h3>
                </div>
            </div>
            <div class="d-flex">
                <div>
    ...
    <div style="padding: 30px" class="ml-5">
        ...
        <div class="d-flex">
            <textarea maxlength="40" id="motivation" class="edit-form__field ember-text-field ember-view" type="text"
                      style="background-color: #1f364d;color: #dde1e4;border-radius:3px;font-size: 18px;padding: 14px;transition: border-width 120ms;width: 50%;border: none"
                      rows="3">{{$productDetails->motivation ?? ''}}</textarea>
        </div>
    ...
    <div class="row p-3" style="margin-left: 63px">
        <div class="d-flex">
            <input type="month" id="start-month" name="bdaymonth" class="w-100"
                   ...
                   value="{{$productDetails->start_date_month ?? ''}}">
        </div>
        <div class="d-flex">
    ...
    <div style="padding: 30px" class="ml-5">
        ...
        <div class="d-flex">
            <input maxlength="40" id="location" class="edit-form__field ember-text-field ember-view" type="text"
                   style="background-color: #1f364d;color: #dde1e4;border-radius:3px;font-size: 18px;padding: 14px;transition: border-width 120ms;width: 50%;border: none"
                   placeholder="Search..." value="{{$productDetails->location ?? ''}}">
        </div>
    ...
    <script>
        ...
        function saveProduct() {
            let myTags = JSON.stringify(tagsArray);
            let myPlatforms = JSON.stringify(platformsArray);
            let funding = JSON.stringify(fundingArray);
            let businessArray = JSON.stringify(BusinessArray);
            let name = document.getElementById('name').value;
            let motivation = document.getElementById('motivation').value;
            let tagline = document.getElementById('tagline').value;
            let webLink = document.getElementById('web-link').value;
            let twitterLink = document.getElementById('twitter-link').value;
            ...
            if (tagline === '') {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'Tagline section cannot be empty!',
                });
                return;
            }
            if (motivation === '') {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'Motivation section cannot be empty!',
                });
                return;
            }
            if (webLink === '' || webLink === null) {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'Web link section cannot be empty!',
                });
                return;
            }
            if (!/(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,})/gi.test(webLink)) {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'Invalid URL!',
            ...
            if (startMonth === '' || startMonth === null) {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'Start month cannot be empty!',
                });
                return;
            }
            ...
            if (endMonth === '' || endMonth === null) {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'End month cannot be empty!',
                });
                return;
            }
            if (endYear === '' || endYear === null) {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'End year cannot be empty!',
                });
                return;
            }
            $.ajax({
                url: `{{env('APP_URL')}}/products/details/save`,
                type: 'POST',
                dataType: "JSON",
                data: {
                    name: name,
